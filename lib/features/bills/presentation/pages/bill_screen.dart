...
import '../../../../di/di.dart';
import '../manager/bill_bloc.dart';
import '../manager/bill_state.dart';
import 'dart:convert';
...
class BillsScreen extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final billBloc = useMemoized(() => getIt<BillBloc>());

    useEffect(() {
      context.read<BillBloc>().add(BillEvent.fetchBills());
      return null;
    }, []);

    return Scaffold(
      body: RefreshIndicator(
        onRefresh: () async => billBloc.add(BillEvent.fetchBills()),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: Text('DATA TAGIHAN', style: context.shadTextTheme.h2),
              ),
    ...
    return Transform.translate(
      ...
      child: FBreadcrumb(
        children: [
          ...
          FBreadcrumbItem(current: true, child: const Text('Tagihan')),
        ],
      ),
    );
  }
  ...
  Widget _buildDataTable(
    ...
    return Skeletonizer(
      enabled: state.status == BillStatus.loading,
      child: Theme(
        ...
        child: Container(
          ...
          child: PlutoGrid(
            columns: [
              ...
              PlutoColumn(
                title: 'ID TAGIHAN SISWA',
                field: 'student_fee_id',
                type: PlutoColumnType.text(),
                hide: true,
              ),
              PlutoColumn(
                title: 'ID JENIS TAGIHAN',
                field: 'fee_type_id',
                type: PlutoColumnType.text(),
                hide: true,
              ),
            ...
            rows:
                bills.map((bill) {
                  return PlutoRow(
                    cells: {
                      'bill_id': PlutoCell(value: bill.billId),
                      'student_id': PlutoCell(value: bill.studentId),
                      'student_fee_id': PlutoCell(value: bill.studentFeeId),
                      'fee_type_id': PlutoCell(value: bill.feeTypeId),
                      'bill_name': PlutoCell(value: bill.billName),
                      'bill_month': PlutoCell(value: bill.billMonth),
                      'amount': PlutoCell(value: bill.amount),
                      'status': PlutoCell(value: bill.status),
                      'merchant_code': PlutoCell(value: bill.merchantCode),
                      'reference': PlutoCell(value: bill.reference),
                      'payment_url': PlutoCell(value: bill.paymentUrl),
                      'created_at': PlutoCell(value: bill.createdAt),
                    },
                  );
                }).toList(),
            onLoaded: (event) {
