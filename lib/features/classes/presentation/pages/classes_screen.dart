...
import '../manager/class_bloc.dart';
import '../manager/class_event.dart';
import '../manager/class_state.dart';

class ClassesScreen extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final schoolBloc = useMemoized(() => getIt<ClassBloc>());

    useEffect(() {
      context.read<ClassBloc>().add(ClassEvent.fetchClass());
      return null;
    }, []);

    return Scaffold(
      body: RefreshIndicator(
        onRefresh: () async => schoolBloc.add(ClassEvent.fetchClass()),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: Text('DATA KELAS', style: context.shadTextTheme.h2),
              ),
    ...
    return Transform.translate(
      ...
      child: FBreadcrumb(
        children: [
          ...
          FBreadcrumbItem(current: true, child: const Text('Kelas')),
        ],
      ),
    );
  }
  ...
  Widget _buildDataTable(
    ...
    return Skeletonizer(
      enabled: state.status == ClassStatus.loading,
      child: Theme(
          ...
          child: PlutoGrid(
            ...
            rows:
                classes.map((_class) {
                  return PlutoRow(
                    cells: {
                      'class_id': PlutoCell(value: _class.classId),
                      'school_id': PlutoCell(value: _class.schoolId),
                      'room_id': PlutoCell(value: _class.roomId),
                      'class_name': PlutoCell(value: _class.className ?? '-'),
                      'description': PlutoCell(value: _class.description ?? '-'),
                      'created_at': PlutoCell(value: _class.createdAt),
                    },
                  );
                }).toList(),
            onLoaded: (event) {
