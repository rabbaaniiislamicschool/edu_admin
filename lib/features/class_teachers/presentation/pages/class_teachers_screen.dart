...
import '../../../../di/di.dart';
import '../manager/class_teacher_bloc.dart';
import '../manager/class_teacher_event.dart';
import '../manager/class_teacher_state.dart';
import 'dart:convert';
...
class ClassTeachersScreen extends HookWidget {
  const ClassTeachersScreen({super.key});
  @override
  Widget build(BuildContext context) {
    final controller = useFPopoverController();
    final stateManager = useState<PlutoGridStateManager?>(null);
    final classTeacherBloc = useMemoized(() => getIt<ClassTeacherBloc>());

    useEffect(() {
      context.read<ClassTeacherBloc>().add(ClassTeacherEvent.fetchClassTeachers());
      return null;
    }, []);
    ...
    return Scaffold(
      body: RefreshIndicator(
        onRefresh:
            () async => classTeacherBloc.add(ClassTeacherEvent.fetchClassTeachers()),
        child: BlocBuilder<ClassTeacherBloc, ClassTeacherState>(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: Text('DATA WALI KELAS', style: context.shadTextTheme.h2),
              ),
    ...
    return Transform.translate(
      ...
      child: FBreadcrumb(
        children: [
          ...
          FBreadcrumbItem(current: true, child: const Text('Wali Kelas')),
        ],
      ),
    );
  }
  ...
  Widget _buildDataTable(
      BuildContext context,
      ValueNotifier<PlutoGridStateManager?> stateManager,
      ClassTeacherState state,
      ) {
    final classTeachers = state.classTeachers ?? [];
    return Skeletonizer(
      enabled: state.status == ClassTeacherStatus.loading,
      child: Theme(
        ...
        child: Container(
          ...
          child: PlutoGrid(
            ...
            rows: classTeachers.map((classTeacher) {
              return PlutoRow(
                cells: {
                  'class_teacher_id': PlutoCell(value: classTeacher.classTeacherId),
                  'class_id': PlutoCell(value: classTeacher.classId),
                  'teacher_id': PlutoCell(value: classTeacher.teacherId),
                  'academic_year_id': PlutoCell(value: classTeacher.academicYearId),
                  'assigned_date': PlutoCell(value: classTeacher.assignedDate),
                },
              );
            }).toList(),
            onLoaded: (event) {
