...
Future<Either<Failure, T>> safeRequest<T>(
  ...
  try {
    ...
    return Left(Failure.network(errorNoInternet));
    ...
    if (error.code == '23505') { // UNIQUE Constraint Violation
      final RegExp regex = RegExp(r'_(.*?)_key');
      final match = regex.firstMatch(error.message);
      final column = match != null ? match.group(1)! : "kolom tidak diketahui";
      return Left(Failure.server('$column ada data yang duplikat, data harus unik tidak boleh sama'));
    }
    return Left(Failure.server(error.message));
  } on StorageException catch (error) {
    return Left(Failure.server(error.message));
  } catch (error) {
    return Left(Failure.unexpected('$errorUnexpected: $error'));
  }
}

Future<Either<Failure, T>> safeLocalOperation<T>(
  ...
  try {
    ...
    if (e.message.contains('corrupted')) {
      ...
      return Left(
        Failure.local(
          'Gagal menyimpan data. Pastikan perangkat Anda memiliki cukup ruang.',
        ),
      );
    } else if (e.message.contains('File system error')) {
      return Left(
        Failure.local(
          'Terjadi kesalahan pada sistem file. Silakan restart aplikasi Anda.',
        ),
      );
    } else {
    ...
    return Left(
      Failure.local('Format data tidak valid. Silakan periksa data Anda.'),
    );
  } on TimeoutException catch (e) {
    return Left(
      Failure.local(
        'Operasi melebihi waktu yang ditentukan. Silakan coba lagi.',
      ),
    );
  } catch (e) {
    print(e);
